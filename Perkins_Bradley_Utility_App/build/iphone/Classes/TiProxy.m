...
//Rare exceptions to indicate a bug in the _perkins_bradley_utility_app code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_perkins_bradley_utility_app = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_perkins_bradley_utility_app]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.perkins_bradley_utility_app.module"
