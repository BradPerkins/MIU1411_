...
NSString * Perkins_Bradley_Utility_App$ModuleRequireFormat = @"(function(exports){"
...
@implementation Perkins_Bradley_Utility_AppObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_perkins_bradley_utility_app);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_perkins_bradley_utility_app gc];
}
...
{
	// create Perkins_Bradley_Utility_App global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Perkins_Bradley_Utility_App" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_perkins_bradley_utility_app = [[Perkins_Bradley_Utility_AppObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_perkins_bradley_utility_app];
	
	NSString *_perkins_bradley_utility_appNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _perkins_bradley_utility_appNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_perkins_bradley_utility_app valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Perkins_Bradley_Utility_App$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.perkins_bradley_utility_app.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.perkins_bradley_utility_app.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
