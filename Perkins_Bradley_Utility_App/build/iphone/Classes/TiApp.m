...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kPerkins_Bradley_Utility_AppUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kPerkins_Bradley_Utility_AppUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a Perkins_Bradley_Utility_App app using the node.js CLI
